...
export async function middleware(request: NextRequest) {
  ...
  const isRoutineHealthProbe = url === "/api/health" || url.startsWith("/api/health/");
  const shouldLogRequest = process.env.NODE_ENV === "development" && !isRoutineHealthProbe;
  ...
  // Style source - the app and Next runtime still rely on inline style attributes
  // (e.g. autosizing textareas, Next-managed image/layout styles). Allow inline
  // styles while keeping script CSP strict via nonce + strict-dynamic.
  const styleSrc = ["'self'", "'unsafe-inline'"];
  
  // style-src-elem also needs to permit inline style tags generated by Next/font
  // and other runtime-managed stylesheets, plus approved external providers.
  const styleSrcElem = [
    "'self'",
    "'unsafe-inline'",
