...
import { MonthlyViolationAIChart } from "@/app/dashboard-echarts/components/complexchart/base/MonthlyViolationAIChart";
import type { MonthlyViolationChartHandle } from "@/app/dashboard-echarts/components/complexchart/base/MonthlyViolationAIChart";
...
type Props = {
  id?: string;
  title?: string;
  chartType?: React.ComponentProps<typeof MonthlyViolationAIChart>["chartType"];
  colors?: string[];
...
export default function Chart07({
  ...
  const [isCollapsed, setIsCollapsed] = useState(() =>
    getCollapsedState("chartCollapsed07", true)
  );
  ...
  useEffect(() => {
    if (
      ...
      console.log(
        "[Chart07] Refreshing chart due to WebSocket update:",
        lastUpdateTime
      );

      // โดยปกติ MonthlyViolationAIChart จะมี method สำหรับ refresh ข้อมูล
  ...
  const toggle = () => {
    ...
    setIsCollapsed(ns);
    setCollapsedState("chartCollapsed07", ns);
  };
