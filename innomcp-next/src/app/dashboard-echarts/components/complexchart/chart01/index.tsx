...
export default function Chart01({
  ...
  const [isCollapsed, setIsCollapsed] = useState(() =>
    getCollapsedState("chartCollapsed01", true)
  );
  ...
  useEffect(() => {
    if (
      ...
      console.log(
        "[Chart01] Refreshing chart due to WebSocket update:",
        lastUpdateTime
      );
  ...
  const toggle = () => {
    ...
    setIsCollapsed(ns);
    setCollapsedState("chartCollapsed01", ns);
  };
  ...
  const handlePreview = () => {
    ...
    onOpenReport({
      ...
      filename: `${chartTitle
        .replace(/\s+/g, "_")
        .toLowerCase()}_bubble_report.docx`,
      valueSuffix: "URL",
      dateRange: { startDate: "", endDate: "" },
      title: chartTitle,
      subtitle: "",
      // chartDivRef may be null initially; cast to RefObject<HTMLDivElement> as downstream expects non-nullable ref
      chartDivRef: chartDivRef as React.RefObject<HTMLDivElement>,
    });
  };

  // handle label click -> fetch URLs and forward to parent modal
