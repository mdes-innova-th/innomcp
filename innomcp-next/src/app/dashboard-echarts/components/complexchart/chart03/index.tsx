...
import DailyViolationChart, {
  type DailyViolationChartHandle,
} from "@/app/dashboard-echarts/components/complexchart/base/DailyViolationChart";
...
type Props = {
  id?: string;
  title?: string;
  chartType?: React.ComponentProps<typeof DailyViolationChart>["chartType"];
  colors?: string[];
  onDataLabelClick?: React.ComponentProps<
    typeof DailyViolationChart
  >["onDataLabelClick"];
...
export default function Chart03({
  ...
  const chartRef = useRef<DailyViolationChartHandle | null>(null);
  const chartDivRef = useRef<HTMLDivElement | null>(null);
  const [loading, setLoading] = useState(true);
  const [ready, setReady] = useState(false);
  const [isCollapsed, setIsCollapsed] = useState(() =>
    getCollapsedState("chartCollapsed03", true)
  );
  ...
  useEffect(() => {
    if (
      ...
      console.log(
        "[Chart03] Refreshing chart due to WebSocket update:",
        lastUpdateTime
      );

      // โดยปกติ DailyViolationChart จะมี method สำหรับ refresh ข้อมูล
  ...
  const toggle = () => {
    ...
    setIsCollapsed(ns);
    setCollapsedState("chartCollapsed03", ns);
  };
