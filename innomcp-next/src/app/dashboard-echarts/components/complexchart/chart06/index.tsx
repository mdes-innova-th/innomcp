...
import {
  DailyViolationAIChart,
  type DailyViolationChartHandle,
} from "@/app/dashboard-echarts/components/complexchart/base/DailyViolationAIChart";
...
type Props = {
  id?: string;
  title?: string;
  chartType?: React.ComponentProps<typeof DailyViolationAIChart>["chartType"];
  colors?: string[];
...
export default function Chart06({
  ...
  const [isCollapsed, setIsCollapsed] = useState(() =>
    getCollapsedState("chartCollapsed06", true)
  );
  ...
  useEffect(() => {
    if (
      ...
      console.log(
        "[Chart06] Refreshing chart due to WebSocket update:",
        lastUpdateTime
      );

      // โดยปกติ DailyViolationAIChart จะมี method สำหรับ refresh ข้อมูล
  ...
  const toggle = () => {
    ...
    setIsCollapsed(ns);
    setCollapsedState("chartCollapsed06", ns);
  };
