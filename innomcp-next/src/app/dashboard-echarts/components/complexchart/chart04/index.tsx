...
import { MonthlyViolationChart } from "@/app/dashboard-echarts/components/complexchart/base/MonthlyViolationChart";
import type { MonthlyViolationChartHandle } from "@/app/dashboard-echarts/components/complexchart/base/MonthlyViolationChart";
...
type Props = {
  id?: string;
  title?: string;
  chartType?: React.ComponentProps<typeof MonthlyViolationChart>["chartType"];
  colors?: string[];
  onDataLabelClick?: React.ComponentProps<
    typeof MonthlyViolationChart
  >["onDataLabelClick"];
...
export default function Chart04({
  ...
  const chartRef = useRef<MonthlyViolationChartHandle | null>(null);
  const chartDivRef = useRef<HTMLDivElement | null>(null);
  const [loading, setLoading] = useState(true);
  const [ready, setReady] = useState(false);
  const [isCollapsed, setIsCollapsed] = useState(() =>
    getCollapsedState("chartCollapsed04", true)
  );
  ...
  useEffect(() => {
    if (
      ...
      console.log(
        "[Chart04] Refreshing chart due to WebSocket update:",
        lastUpdateTime
      );

      // โดยปกติ MonthlyViolationChart จะมี method สำหรับ refresh ข้อมูล
  ...
  const toggle = () => {
    ...
    setIsCollapsed(ns);
    setCollapsedState("chartCollapsed04", ns);
  };
  ...
  const handlePreview = () => {
    ...
    const rows =
      typeof chartRef.current?.getChartData === "function"
        ...
        : [];
    onOpenReport({
      tableHeaders: ["เดือน", "จำนวน URL"],
      tableRows: rows,
      filename: `${chartTitle.replace(/\s+/g, "_").toLowerCase()}.docx`,
      valueSuffix: "URL",
  ...
  const handleLabelClick = async (date: string, groupName: string) => {
    try {
      const params = new URLSearchParams();
      params.append("date", date);
      params.append("groupName", groupName);
      const host = process.env.NEXT_PUBLIC_NODE_BACKEND_HOST || "";
      const endpoint = `${host}/api/urlstats/list-by-date-group?${params.toString()}`;
      const res = await fetch(endpoint, {
        headers: { "Content-Type": "application/json" },
      });
      const data = await res.json();
      onOpenUrlModal?.({
        groupName: `${groupName} (${date})`,
        urls: data.urls || [],
        startDate: date,
        endDate: date,
      });
