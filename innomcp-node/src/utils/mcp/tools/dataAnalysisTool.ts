 ...
 *
 * Designed to be called via the MCP JSON-RPC tools/call endpoint at
 * innomcp-server-node so the conductor can pick it up for intent="data".
 */
...
function inferType(values: unknown[]): ColumnStat["type"] {
  ...
  for (const v of values) {
    ...
    const t = typeof v;
    if (t === "number" || (t === "string" && v !== "" && !isNaN(Number(v as string)))) {
      hasNum = true;
    } else if (t === "boolean") {
      hasBool = true;
    } else {
...
async function execute(args: DataAnalysisInput): Promise<AnalysisResult> {
  const format: "csv" | "json" = args.format ?? "csv";
  ...
  try {
    ...
    throw new Error(`Failed to parse ${format}: ${msg}`);
  }

  if (allRows.length === 0) {
    throw new Error("Data parsed to zero rows");
  }
