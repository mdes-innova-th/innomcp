...
// POST /api/memories  { scope, keyName, value, sessionId?, projectId? }
router.post("/", async (req: Request, res: Response) => {
  const { scope = "session", keyName, value, sessionId, projectId } = req.body;
  ...
  try {
    await withDbConnection(async (conn) => {
      await conn.query(
        `INSERT INTO memories (scope, key_name, value, session_id, project_id)
         VALUES (?, ?, ?, ?, ?)`,
        [scope, keyName, String(value), sessionId ?? null, projectId ?? null]
      );
    });
